...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /monitoring", s.requireSession(s.requireCapability(rbac.ManageServer, s.monitoringPage)))
	mux.HandleFunc("GET /databases", s.requireSession(s.requireCapability(rbac.ManageServer, s.databasesPage)))
	mux.HandleFunc("POST /databases", s.requireSession(s.requireCapability(rbac.ManageServer, s.createDatabase)))
	mux.HandleFunc("POST /databases/phpmyadmin/install", s.requireSession(s.requireCapability(rbac.ManageServer, s.installDatabaseAdmin)))
	...
	mux.HandleFunc("POST /sites/{id}/domain", s.requireSession(s.requireCapability(rbac.ManageAllSites, s.changeSiteDomain)))
	mux.HandleFunc("POST /sites/{id}/domain/retry", s.requireSession(s.requireCapability(rbac.ManageAllSites, s.retrySiteDomain)))
	mux.HandleFunc("POST /sites/{id}/delete", s.requireSession(s.deleteSite))
	...
	mux.HandleFunc("POST /backups/google-drive/start", s.requireSession(s.requireCapability(rbac.ManageServer, s.startGoogleDriveOAuth)))
	// The Google redirect cannot depend on a SameSite=Strict session cookie. A
	// one-time, high-entropy state value identifies and authorizes this callback.
