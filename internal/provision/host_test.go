...
func testHost(t *testing.T, runner Runner) *Host {
	...
	root := t.TempDir()
	return &Host{
		HostingUnitRoot:         filepath.Join(root, "systemd"),
		NodeInstallRoot:         filepath.Join(root, "node"),
		CronRoot:                filepath.Join(root, "cron.d"),
		ProFTPDConfigRoot:       filepath.Join(root, "proftpd"),
		PanelTLSCertPath:        filepath.Join(root, "tls", "panel.crt"),
		PanelTLSKeyPath:         filepath.Join(root, "tls", "panel.key"),
		PostfixConfigPath:       filepath.Join(root, "postfix", "main.cf"),
		PolicyRCDPath:           filepath.Join(root, "policy-rc.d"),
		SecurityNginxRoot:       filepath.Join(root, "security", "sites"),
		SecurityNginxGlobal:     filepath.Join(root, "security", "zones.conf"),
		SecurityFail2banJails:   filepath.Join(root, "fail2ban", "jail.d"),
		SecurityFail2banFilters: filepath.Join(root, "fail2ban", "filter.d"),
		AccessRoot:              filepath.Join(root, "access"),
