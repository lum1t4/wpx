...
func TestMagicLoginWritesShortLivedSiteOwnedBootstrap(t *testing.T) {
	for _, scenario := range []struct {
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			site := model.Site{ID: "example-com", Domain: "example.com", Kind: model.WordPress, PHPVersion: "8.4", TLSStatus: scenario.tlsStatus}
			...
			baseURL := scenario.scheme + "://example.com/"
			if !strings.HasPrefix(url, baseURL+"wpx-login-") || !strings.HasSuffix(url, ".php") {
