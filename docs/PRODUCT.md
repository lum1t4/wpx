...
| Operator alerts and stability | Encrypted email, Slack and Telegram channels, separate condition switches, channel cooldown and recovery checks, opt-in managed swap, OOM/crash diagnostics. | Provider-contract and resource fixtures, confined certificate reads, bounded diagnostics. Live provider delivery needs operator credentials; real pressure handling remains a separate host exercise. See [alerts](dispatch/operator-alerts.md). |
